    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {!isEditing ? (
              <button
                onClick={handleEdit}
                className="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-200 hover:scale-105"
              >
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={handleSave}
                  className="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 hover:scale-105"
                >
                ...
                <button
                  ...
                  className="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all duration-200 hover:scale-105"
                >
                  <X className="w-4 h-4" />
      ...
      <main className="px-3 sm:px-6 py-3 sm:py-6">
        <div className="max-w-6xl mx-auto">
          {/* Profile Header */}
          <div className={`bg-white rounded-lg border border-gray-200 p-4 sm:p-6 lg:p-8 mb-4 sm:mb-6 transform transition-all duration-700 ${
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
              <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 w-full sm:w-auto">
                <div className="relative group flex-shrink-0">
                  {profile.profile.avatar ? (
                    <img
                      ...
                      className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover transition-transform duration-300 group-hover:scale-105"
                    />
                  ) : (
                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xl sm:text-2xl font-bold transition-transform duration-300 group-hover:scale-105">
                      {profile.displayName.charAt(0).toUpperCase()}
                    </div>
                  )}
                  {isEditing && (
                    <button className="absolute -bottom-1 -right-1 sm:-bottom-2 sm:-right-2 w-7 h-7 sm:w-8 sm:h-8 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 transition-colors duration-200">
                ...
                <div className="flex-1 text-center sm:text-left w-full sm:w-auto">
                  {!isEditing ? (
                    <>
                      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{profile.displayName}</h1>
                      <p className="text-gray-600 mb-4 leading-relaxed text-sm sm:text-base">{profile.profile.bio}</p>
                    </>
                  ) : (
                    <div className="space-y-4 w-full">
                      <input
                        ...
                        className="text-2xl sm:text-3xl font-bold text-gray-900 bg-transparent border-b-2 border-gray-300 focus:border-blue-500 outline-none transition-colors duration-200 w-full"
                      />
                      <textarea
                        value={editForm.bio}
                        onChange={(e) => setEditForm(prev => ({ ...prev, bio: e.target.value }))}
                        rows="3"
                        className="w-full text-gray-600 bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base"
                        placeholder="Tell us about yourself..."
                      />
                    </div>
                  )}
                  {/* Contact Info - Stack on mobile, inline on larger screens */}
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-6 text-sm text-gray-600">
                    ...
                    {(profile.profile.location || isEditing) && (
                      <div className="flex items-center justify-center sm:justify-start space-x-1">
                        ...
                        {!isEditing ? (
                          <span className="truncate">{profile.profile.location}</span>
                        ) : (
                          <input
                            type="text"
                            value={editForm.location || ''}
                            onChange={(e) => setEditForm(prev => ({ ...prev, location: e.target.value }))}
                            className="bg-transparent border-b border-gray-300 focus:border-blue-500 outline-none transition-colors duration-200 text-center sm:text-left"
          ...
          {/* Stats Grid */}
          <div className={`grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6 transform transition-all duration-700 delay-100 ${
            isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
          }`}>
            {[
              ...
              <div key={stat.label} className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 hover:shadow-md transition-shadow duration-300">
          ...
          {/* Tabs */}
          <div className={`bg-white rounded-lg border border-gray-200 mb-4 sm:mb-6 transform transition-all duration-700 delay-200 ${
            isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
          }`}>
              ...
              <nav className="flex overflow-x-auto scrollbar-hide px-3 sm:px-6">
                {[
                  ...
                  <button
                    ...
                    className={`py-3 sm:py-4 px-3 sm:px-1 mr-6 sm:mr-8 border-b-2 font-medium text-sm whitespace-nowrap transition-all duration-200 ${
                      activeTab === tab.id
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 space-y-1 sm:space-y-0">
                          ...
                          <span className="font-medium text-gray-900 text-sm sm:text-base sm:text-right">{profile.profile.college}</span>
                        </div>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 space-y-1 sm:space-y-0">
                          <span className="text-gray-600 text-sm">Role</span>
                          <span className="font-medium text-gray-900 capitalize text-sm sm:text-base">{profile.role}</span>
                        </div>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 space-y-1 sm:space-y-0">
                          <span className="text-gray-600 text-sm">Volunteering Hours</span>
                          <span className="font-medium text-gray-900 text-sm sm:text-base">{profile.stats.volunteeringHours}</span>
                        </div>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 space-y-1 sm:space-y-0">
                          <span className="text-gray-600 text-sm">Projects Completed</span>
                          <span className="font-medium text-gray-900 text-sm sm:text-base">{profile.stats.projectsCompleted}</span>
                        </div>
                      </div>
                    </div>
                    <div>
                          ...
                          <>
                            {profile.profile.githubUrl && (
                              <a href={profile.profile.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <span className="text-gray-900 truncate">GitHub Profile</span>
                              </a>
                            )}
                            {profile.profile.linkedinUrl && (
                              <a href={profile.profile.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <span className="text-gray-900 truncate">LinkedIn Profile</span>
                              </a>
                            )}
                            {profile.profile.portfolioUrl && (
                              <a href={profile.profile.portfolioUrl} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <span className="text-gray-900 truncate">Portfolio Website</span>
                              </a>
                            )}
                          </>
                        ) : (
                ...
                <div className="animate-fadeIn">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isEditing && (
                      <button
                        onClick={addSkill}
                        className="flex items-center space-x-1 px-3 py-1 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200 text-sm"
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    {(isEditing ? editForm.skills : profile.profile.skills).map((skill, index) => (
                      <div key={index} className="relative group">
                        {!isEditing ? (
                          <div className="px-3 py-2 bg-blue-50 text-blue-800 rounded-lg text-sm font-medium hover:bg-blue-100 transition-colors duration-200 text-center">
                            {skill}
                          </div>
                        ) : (
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors duration-200 flex-shrink-0"
                            >
